...
2. **Every country opens the same country page** (revised 2026-08-18 — see below; previously branched on content availability into a separate coming-soon dead end)
   ...
   **2026-08-18, per Colleen: "instead of the coming soon page we should just have a country page showing any available data where the expanded categories say 'coming soon.'"** The old dedicated coming-soon screen (short message + generic resource links, `coming_soon_resources` table) is retired from this flow — see HANDOFF.md for the implementation. **Resolved the same day**: those generic resource links resurface as their own standalone accordion section, "Additional Resources" — the seventh and last section on every country's page (not just uncovered ones, and not nested inside another section's empty state) — rather than being dropped. See HANDOFF.md's entry for the implementation.
...
- ~~**Reopened 2026-08-18: do coming-soon resource links resurface, and where?**~~ — **resolved, same day, per Colleen:** as their own "Additional Resources" accordion section (the seventh, always last) rather than nested inside another section's empty state — simpler than the per-section resource sets nesting them into e.g. Language's "Coming soon" body would have needed, and useful on every country's page, not just uncovered ones.
