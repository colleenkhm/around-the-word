...
- **`ContinentMapScreen` (the world map) now opens pannable/pinchable via a computed initial `Matrix4`, instead of a bare, un-positioned `InteractiveViewer`** — changed 2026-07-28 at Colleen's request. Landed on: whole map visible and dead-centered (`fitScale`, `min` of viewport width/height ratios vs. map size — also the zoom-out floor via `minScale`), computed from the viewport at runtime rather than hardcoded so it holds up across device sizes. (Two things were tried and rejected along the way: zooming in past "whole map visible," and anchoring the initial position toward the bottom edge instead of centering — the latter looked fine in theory but on a phone-sized viewport the map is small enough relative to the screen that "biased toward the bottom" read as "stuck to the bottom edge." Worth knowing before re-attempting either.) Reused `CountryMapScreen`'s existing `constrained: false` + `TransformationController` pattern rather than inventing a second approach, just centered on the whole map instead of one continent's bounds. Pulled the map's native pixel size (`Size(2000, 857)`, from the package's embedded "w"/"h") out of `country_map_screen.dart` into a shared `mapSize` constant in `lib/data/continent_zoom_bounds.dart` since both screens' zoom math now depends on it. `lib/map_prototype/continent_tap_screen.dart` was deliberately left as-is (unzoomed, full world, dead-centered via `FittedBox`) — it already documents itself as a superseded reference copy, not something this needs to stay in sync with.

- **`ContinentMapScreen` (the landing screen only, not the other four in the flow) traded its plain `AppBar` for a custom header + repositioned title** — added 2026-07-28 at Colleen's request: two `TextButton`s ("About", "Resources") right-aligned in a header row, then extra spacing before the "Where are you going?" heading (now body text via `Theme.of(context).textTheme.headlineSmall`, not an `AppBar` title) above the map. Backing screens are `lib/screens/about_screen.dart` and `resources_screen.dart` — both intentionally bare placeholders (`Scaffold` + centered "coming soon" text), no content specified yet. Scoped to just this screen on purpose (confirmed with Colleen before building): the other four screens' `AppBar`s carry step-specific titles/back buttons (continent name, "Categories", etc.) that a shared global header would've displaced or duplicated, and nothing asked for that.

- **Continent taps that miss every drawn country path now have a geographic fallback for the Central American isthmus**, rather than always showing "Unrecognized region" — added 2026-07-28 after Colleen hit it tapping near Mexico. Root cause: `SimpleMapPainter` (in the `countries_world_map` package) draws a full-canvas transparent background *first*, and any tap that doesn't land inside a real country polygon fires `callback("", "", ...)` off that background instead — confirmed the package's own embedded map data has exactly one path per country (219 unique ids, matching our dataset exactly, so this isn't missing/drifted data), the small Central American countries just leave real unrendered gaps between them at typical zoom levels. Fixed by giving `resolveContinentTap` (`lib/data/map_tap_resolution.dart`) optional `tapPosition`/`fallbackBounds`/`fallbackContinent` params: when the tapped id doesn't match any country *and* a tap position is given that falls inside the bounds, it resolves to the fallback continent instead of failing — a real id match still always wins. `ContinentMapScreen` passes `details.localPosition` (converted to the same 0..1 fraction space `continentZoomBounds` already uses) plus a hand-computed `centralAmericaFallbackBounds` rect (`lib/data/continent_zoom_bounds.dart`, padded out from the real bounding box of Belize/Guatemala/Honduras/El Salvador/Nicaragua/Costa Rica/Panama's path data) and `'north-america'`. Deliberately scoped to the continent-picker only, not `resolveCountryTap`/`CountryMapScreen` — at the country level a background-gap tap would need to resolve to one specific real country, not a generic "Central America," so the same trick doesn't apply there.
